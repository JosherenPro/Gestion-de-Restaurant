...
import { LayoutDashboard, Users, Utensils, Table as TableIcon, FileText, Settings, TrendingUp, DollarSign, ShoppingBag, Plus, MoreHorizontal, Trash2, Edit } from 'lucide-react';
import { Card, Button, Modal } from './UI';
import { MENU_ITEMS, STAFF, TABLES } from '../mockData';
        ...
        {currentView === 'DASHBOARD' && (
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                { label: 'Recettes du jour', value: '€4,592.00', icon: DollarSign, color: 'bg-green-100 text-green-600', trend: '+12.5%' },
                { label: 'Commandes totales', value: '148', icon: ShoppingBag, color: 'bg-blue-100 text-blue-600', trend: '+5.4%' },
                { label: 'Clients satisfaits', value: '98%', icon: Users, color: 'bg-orange-100 text-orange-600', trend: '+2.1%' },
                { label: 'Panier moyen', value: '€31.00', icon: TrendingUp, color: 'bg-purple-100 text-purple-600', trend: '-1.4%' },
              ].map((stat, idx) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              ...
              <Card className="p-10 bg-[#03081F] text-white border-none shadow-2xl rounded-[2.5rem] relative overflow-hidden">
                ...
                <div className="space-y-8 relative z-10">
                  {MENU_ITEMS.slice(0, 4).map((plat, idx) => (
                    <div key={idx} className="flex items-center gap-5 group">
                      <img src={plat.image} className="w-14 h-14 rounded-2xl object-cover shadow-lg border-2 border-white/10 group-hover:scale-105 transition-transform" />
                      <div className="flex-1">
                        <p className="font-bold text-sm tracking-tight">{plat.name}</p>
                        <p className="text-[10px] text-gray-500 font-bold uppercase">{plat.category}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-black text-[#FC8A06]">€{plat.price.toFixed(2)}</p>
                        <p className="text-[10px] text-gray-500 font-bold">142 vendus</p>
                      </div>
                    </div>
                  ))}
                </div>
               ...
               <tbody className="divide-y divide-gray-50">
                 {MENU_ITEMS.map(item => (
                   <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                     ...
                     <td className="px-8 py-6">
                        <span className="font-black text-[#03081F]">€{item.price.toFixed(2)}</span>
                     </td>
                     <td className="px-8 py-6">
